...
class MyUserManager(BaseUserManager):
    def create_user(self, email, name, user_name, password=None):
        ...
        user = self.model(
            email=self.normalize_email(email), user_name=user_name, name=name
        )
...
class GmailAPPModel(models.Model):
    user = models.ForeignKey(
        NewUserRegistration, on_delete=models.CASCADE, null=True, blank=True
    )
...
@receiver(pre_save, sender=NewUserRegistration)
def revoke_tokens(sender, instance, **kwargs):
    if not instance._state.adding:
        existing_user = NewUserRegistration.objects.get(pk=instance.pk)
        if (
            instance.password != existing_user.password
            or instance.email != existing_user.email
            or instance.user_name != existing_user.user_name
        ):
